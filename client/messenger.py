...
class MessengerApp(clientui.Ui_MainWindow):
    ...
    def sendMessege(self):
        ...
        response = requests.post('http://localhost:10000/send',
    ...
    def getUpdates(self):
        '''response = requests.post('http://localhost:10000/history',
                                 json={"after": self.last_message_time})'''
        if self.last_message_time != 0:
            response = requests.get(
                'http://localhost:10000/history',
